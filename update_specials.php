...
// Check if specials to update is selected
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_id'])) {
    $update_id = $_POST['update_id'];
    // Fetch specials data from the database
    $sql = "SELECT * FROM specials WHERE id = '$update_id'";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) == 1) {
        $special = mysqli_fetch_assoc($result);
    } else {
        // specials not found, redirect back to admin_specials.php
        $_SESSION['message'] = "Specials not found.";
        header("Location: admin_specials.php");
        exit();
    }
} else {
    // If update_id is not set, redirect back to admin_specials.php
...
// Check if form for updating special is submitted
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_special'])) {
    ...
    if ($start_date > $end_date) {
        ...
        // Update specials in the database
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            // Redirect back to admin_specials.php after 4 seconds
            ...
            <div class="promotion-container">
                ...
                <form class="promotion-form" id="updateForm" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="price" name="price" value="<?php echo $special['price']; ?>" min="0" step="0.01" required>
                    </div>
                    <div class="form-group">
